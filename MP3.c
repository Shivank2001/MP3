            ...
            while (1) {
                ...
                if (activity < 0) {
                    ...
                    if (ack_msg.opcode == htons(4) && ack_msg.block_no == block_no) {
                        ...
                        block_no = (block_no == 65535) ? 0 : block_no + 1;  // Implement wraparound
...
int main(int argc, char *argv[]) {
    ...
    for (p = res; p != NULL; p = p->ai_next) {
        ...
        // Initialize address structure based on IP version
    ...
    while (1) {
        ...
        if (recv_len > 0) {
            ...
            if (pid < 0) {
                ...
                new_addr.sin_port = 0;  // Let the system assign a port

                if (bind(new_sockfd, (struct sockaddr*)&new_addr, sizeof(new_addr)) < 0) {
                    perror("bind failed in child process");
                    close(new_sockfd);
                    exit(EXIT_FAILURE);
                }
                ...
                if (opcode == 1) {
                    ...
                    error_msg.opcode = htons(5);
                    error_msg.Errorcode = htons(4);
                    strncpy(error_msg.Error_msg, "Unknown opcode", sizeof(error_msg.Error_msg));
                    sendto(new_sockfd, &error_msg, sizeof(error_msg), 0, (struct sockaddr *)&client_addr, client_len);
                }
